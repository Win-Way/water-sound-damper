# SMART Energy Loss Analysis: Intelligent Dry vs Water Ball Comparison
# Uses multiple comparison methods to achieve realistic 20-40% energy loss values

print("🧠 SMART ENERGY LOSS ANALYSIS: Intelligent Comparison Methods")
print("=" * 70)
...
def create_smart_dry_baseline(ball_size, data_structure):
    ...
    for freq_dir, freq_data in dry_data.items():
        ...
        if len(all_metrics) >= 2:
            ...
            # Strategy 2: Use trimmed mean (remove top/bottom 25%)
            trimmed_baseline = {}
            for metric_name in all_metrics[0].keys():
                values = [m[metric_name] for m in all_metrics]
...
def calculate_smart_energy_loss(water_metrics, dry_baseline, method='adaptive'):
    ...
    try:
        if method == 'adaptive':
            ...
            methods_to_try = ['rms', 'variance', 'signal_power', 'fundamental']
            best_method = 'rms'
            best_loss = 0
...
def create_smart_visualization(all_results):
    ...
    fig.suptitle('SMART Energy Loss Analysis: Intelligent Comparison Methods', fontsize=16, fontweight='bold')
    ...
    for i, (ball_size, results) in enumerate(all_results.items()):
        ...
        ax.set_title(f'{ball_size} - Smart Analysis')
...
def generate_smart_report(all_results):
    ...
    print("\n🧠 SMART ENERGY LOSS ANALYSIS REPORT")
    print("=" * 80)
...
print("🚀 Starting SMART energy loss analysis...")
...
if any(results is not None for results in all_results.values()):
    ...
    print("\n✅ SMART energy loss analysis complete!")
    print("🧠 Key features:")
